...
namespace BanVeRapChieuPhim.GUI_QuanLyLichChieu
{
    public partial class GUI_KTG : Form
    {
        ...
        private void btnTimKiem_Click(object sender, EventArgs e)
        {
            if (txtTimKiem.Text != "")
            {
                //string[] check = txtTimKiem.Text.Split('/');
                //if (check.Length > 0)
                //{
                //    DateTime ngayChieu = BUS.XuLyNgayThang.XuLyChuoiNgayThang(txtTimKiem.Text);
                //    dgvKTG.DataSource = BUS.BUS_QuanLyLichChieu.BUS_KhungThoiGian.Search(ngayChieu, "");
                //}
                //else
                //{
                    dgvKTG.DataSource = BUS.BUS_QuanLyLichChieu.BUS_KhungThoiGian.Search(txtTimKiem.Text, txtTimKiem.Text);
                //}
